...
pub fn export_excel(data: &FormData, computed: &CalcResult, path: &str) -> Result<(), String> {
    ...
    // ── Sheet: Settlement Report (bank type only) ──
    if !is_import {
    ...
    let doc_items: Vec<(&str, String)> = vec![
        ...
        ("Date", today_str),
        ("Doc Type", if is_import { "Import".into() } else { "Bank".into() }),
    ...
    // Section 2: Computed Values (bank type only)
    if !is_import {
        sheet2
        ...
        // Section 3: Invoice Details (bank type only)
        sheet2
    ...
    if is_import {
        ...
        // Document info
        sheet3.write_with_format(r3, 0, format!("Doc: {}  |  Type: Import", data.doc_serial), &info_fmt)
            .map_err(|e| e.to_string())?;
        r3 += 2;
